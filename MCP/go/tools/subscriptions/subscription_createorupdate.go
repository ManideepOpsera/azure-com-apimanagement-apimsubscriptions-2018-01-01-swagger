...
func Subscription_createorupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		sidVal, ok := args["sid"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: sid"), nil
		}
		sid, ok := sidVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: sid"), nil
		}
		...
		if val, ok := args["notify"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("notify=%v", val))
		}
		...
		var requestBody models.SubscriptionCreateParameters
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/subscriptions/%s%s", cfg.BaseURL, resourceGroupName, serviceName, sid, subscriptionId, queryString)
		...
		var result models.SubscriptionContract
...
func CreateSubscription_createorupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_subscriptions_sid",
		mcp.WithDescription("Creates or updates the subscription of specified user to the specified product."),
		...
		mcp.WithString("sid", mcp.Required(), mcp.Description("Subscription entity Identifier. The entity represents the association between a user and a product in API Management.")),
		mcp.WithBoolean("notify", mcp.Description("Notify change in Subscription State. \n - If false, do not send any email notification for change of state of subscription \n - If true, send email notification of change of state of subscription ")),
		...
		mcp.WithObject("properties", mcp.Description("Input parameter: Parameters supplied to the Create subscription operation.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Subscription_createorupdateHandler(cfg),
	}
}
