...
func Subscription_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/subscriptions%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, queryString)
...
func CreateSubscription_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_subscriptions",
		mcp.WithDescription("Lists all subscriptions of the API Management service instance."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Subscription_listHandler(cfg),
	}
}
